...
def main():
    ...
    parser.add_argument("--check", action="store_true", help="verify checked-in packages without modifying them")
    args = parser.parse_args()
    qsb = shutil.which("qsb")
    if not qsb:
        raise SystemExit("qsb is required; run make shaders in the Qt development environment")
    if args.check:
