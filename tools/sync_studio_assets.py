...
Wraps the shared JavaScript modules referenced by the website, extracting
top-level exports dynamically. Desktop-only modules are not published.
...
def outputs():
    ...
    modules = re.findall(
        r'<script src="assets/studio/([^"/]+\.js)"',
        (ROOT / "docs/website/index.html").read_text(),
    )
    for name in modules:
        if name in result:
            continue
        ...
        if not source.exists():
            source = SOURCE_STUDIO / name
        result[name] = wrap_qml_js(source.read_text(), source.stem)
