...
Auto-discovers and wraps any .js files in package/contents/code/ and
package/contents/ui/studio/, extracting top-level exports dynamically.
...
def outputs():
    ...
    result = {
        ...
        "PresetCodec.js": (SOURCE_STUDIO / "PresetCodec.js").read_bytes(),
        "icon.png": (ROOT / "package/icon.png").read_bytes(),
        "QtShim.js": qt_shim_code(),
    }
    ...
    result["ConfigSchema.js"] = schema_js.encode("utf-8")

    # Auto-discover and sync all JS files in package/contents/code/
    if SOURCE_CODE.exists():
        for js_file in sorted(SOURCE_CODE.glob("*.js")):
            module_name = js_file.stem
            source_text = js_file.read_text("utf-8")
            result[js_file.name] = wrap_qml_js(source_text, module_name)

    # Auto-discover and sync other JS files in package/contents/ui/studio/
    if SOURCE_STUDIO.exists():
        for js_file in sorted(SOURCE_STUDIO.glob("*.js")):
            if js_file.name in ("StudioCatalog.js", "PresetCodec.js"):
                continue
            module_name = js_file.stem
            source_text = js_file.read_text("utf-8")
            result[js_file.name] = wrap_qml_js(source_text, module_name)
